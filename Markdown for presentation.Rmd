...
Gemcitabine is a nucleoside analogue used as chemotherapy. It is marketed as Gemzar. The drug replaces one of the building blocks of nucleic acids, in this case cytidine, during DNA replication. The process arrest tumor growth because gemcitabine induces masked chain termination. Gemcitabine is used in various carcinomas: non-small cell lung cancer, pancreatic cancer, bladder cancer and breast cancer. It is being investigated for use in oesophageal cancer, and is used experimentally in lymphomas and various other tumor types. 
...
The first step is to calculate the absolute value of the fold change, to find the difference in expression of every single gene between treated and untreated cell lines. This way we can detect the biggest changes, regardles of if the expession of the genes was up- or down-regulated.
...
barplot(matrixtopgenes[1:30,], xlab = "Genes", ylab = "Counts", 
...
```{r eval=FALSE}
fcge_gemcitabine <- dplyr::select(as_tibble(fcgeneexpression), contains("gemcitibine"))
...
barplot(meanfcge_gemcitabine, xlab = "Biomarkers", ylab = "Fold change", ylim = c(-1, 3), 
...
Biomarker | Function
----------|-------------
ATF3      | Activating transcription factor, highly expressed in cancer cells
CXCL8     | Interleukin-8: immune activation and angiogenesis
GADD45A   | Cell response to DNA damage
TNFAIP3   | Immune and inflammatory response
CDKN1A    | Regulation of cell cycle and DNA damage repair
GDF15     | Response to inflammation and oxidative stress
BTG2      | Antiproliferative 
EGR1      | Cancer suppressor gene
GEM       | Regulator of signal transduction
IL11      | Interleukin-11: development of B-cells.
TNFRSF9   | Activation and development of T-cells.
CEACAM1   | Regulates angiogenesis, apoptosis, tumor suppression and metastasis.
ANKRD1    | Immune-induced transcription factor
DHRS2     | Enzyme to process xenobiotics and other compounds.
BIRC3     | Inhibits apoptosis
TP53I3    | Tumor supressor
TXNIP     | Tumor supressor 
...
#plotting mutations against cell line
...
It can be observed from the previous plot that the MSI-H cells tend to have a higher number of mutations than MSI-L cells. 
...
mutations of genes that play an important role in gemcitabines mechanism of action. One enzyme that plays an important role in converting gemcitabine 
...
monophosphate. The gene encoding this enzyme is called DCK.(Saiki et al., 2012),(Nakano et al., 2015)
...
Nakano, T., Saiki, Y., Kudo, C., Hirayama, A., Mizuguchi, Y., Fujiwara, S., Soga, T., Sunamura, M., Matsumura, N., Motoi, F., et al. (2015). Acquisition of chemoresistance to gemcitabine is induced by a loss-of-function missense mutation of DCK. Biochemical and biophysical research communications 464, 1084-1089.  
Saiki, Y., Yoshino, Y., Fujimura, H., Manabe, T., Kudo, Y., Shimada, M., Mano, N., Nakano, T., Lee, Y., Shimizu, S., et al. (2012). DCK is frequently inactivated in acquired gemcitabine-resistant human cancer cells. Biochemical and biophysical research communications 421, 98-104.
...
1. The drug sensitivity of the celllines (EC50 value) and the gene expression of their biomarkers when
untreated.  
...
rownames(genexp.drugsens) <- c(rownames(genexp.drugsens)[1:nrow(genexp.drugsens)-1], "DS_Gemcitabine")
...
rownames(copynum.drugsens) <- c(rownames(copynum.drugsens)[1:nrow(copynum.drugsens)-1], "DS_Gemcitabine")
...
This multiple regression has an R-squared value of 0.314, which is higher than what each individual biomarker by itself had, but still not enough to be used as a predictor of the drug sensitivity of the celllines.
...
This regression has the highest R-squared value of all, but at 0.544 it still cannot be used to predict
