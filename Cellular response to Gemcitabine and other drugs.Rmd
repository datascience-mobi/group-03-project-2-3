...
```{r}
boxplot(treated, main = "geneexpression after treatment")
...
ylab = "PC 2", main = "PCA of change of the Fold Change")

plot(pca_FC$rotation[, 3], pca_FC$rotation[, 4], col = metadata[1:ncol(untreated), "drug"], pch = 19, xlab = "PC 3", 
ylab = "PC 4", main = "PCA of the Fold Change")

#Try to plot with legend, works with r but not in the markdown doc

#plotting PCA of FC drug associated, PCA 1-2
...
plot(pca_FC$rotation[, 1], pca_FC$rotation[, 2], pch = 19, xlab = "PC1",ylab = "PC2", col=drug_as_factor, main = "FC")
legend("bottomleft", inset = c(-0.4,0),levels(drug_as_factor), xpd = TRUE, pch=19, col = levels)
```
...
```{r}
#check if randomly choosed gene is expressed normalized
...
ht_untreated_MSIH = Heatmap(geneexpression_MSI_related_untreated_MSIH, name = "geneexpression", column_title = "MSI-High", column_names_gp = gpar(fontsize = 5))
ht_untreated_MSIL = Heatmap(geneexpression_MSI_related_untreated_MSIL, name = "geneexpression", column_title = "MSI-Low", column_names_gp = gpar(fontsize = 5))
