...
barplot(correlations.cn.genexp, ylim = c(-1,1), las = 2, cex.names = 0.8, ylab = "Correlation coefficient", main = "Correlation between gene expression and copy number" )
...
Since the correlation of the two variables is so low, we will proceed with a separate linear regressions for:  
...
The drug sensitivity matrix contains the GI50 values of several celllines when exposed to different drugs. First of all, we make sure to leave only the celllines that are contained in the other two matrices for the regression.
...
```{r}
lrATF3genexp <- lm(DS_gemcitibine~ATF3, data = as.data.frame(genexp.drugsens))
summary(lrATF3genexp)
plot(DS_gemcitibine~ATF3, data = as.data.frame(genexp.drugsens))
abline(lm(DS_gemcitibine~ATF3, data = as.data.frame(genexp.drugsens)), col = "blue")
```
...
for (i in 1:length(commonbiomarkers.lr)) {
  r.squared.i <- 
    summary(lm(DS_gemcitabine~genexp.drugsens[,i], data = as.data.frame(genexp.drugsens)))$r.squared
  all.r.squared <- c(all.r.squared, r.squared.i)
}

names(all.r.squared) <- colnames(genexp.drugsens[,1:ncol(genexp.drugsens)-1])

barplot(all.r.squared, ylim = c(0,1), ylab = "R-squared value", main = "Linear regressions using gene expression", cex.names = 0.7, las = 2)
