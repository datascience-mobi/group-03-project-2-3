...
The first step is to calculate a fold change to find the difference in expression of every single gene between treated and untreated cell lines.
...
MSI is defined as a mistake in MMR (Mismatch Repair system). The defect is caused by a downregulation in the gene MLH1 and MSH6. This result in more mistakes while transcribing the DNA, so more mutations will occur. If we plot the number of mutations against the celllines, MSI-H celllines are supposed to have more mutations, to check this, we do the following plot
...
It can be observed that MSI status affects the number of mutations, due to mutations in the repair system. From this observation arises the following question: does MSI status also affect the effect of gemcitabine? MSI-H cancers have a better prognosis when it comes to immune activating therapies, because of new immunogenic antigens, being produced. But does it also affect non-immune mechanisms in the cell? A loss of system which repairs wrong parts could also lead to an increased proliferation of the cell on the one hand because it would destroy itself. On the other hand a faulty repair system could also cause a better proliferation, because it would be easier to "develop" drug escape mechanisms. At first we want to see if there is any visible relationship between the sensitivity of gemcitabine and the MSI status. So we plot the sensitivity against the celllines and colour them due to MSI status.
...
To investigate wether MSI status affects the fold change after gemcitabine treatment, we have to explore the fold change of our 17 biomarkers for gemcitabine in the MSI-High and MSI-Low celllines. Furthermore, the question arises of wether the gene expression of key genes involved in the DNA repair mechanisms is also affected. To this we picked MLH1 and MSH6, which should theoretically be downregulated.

```{r}
#Select the columns names that contain gemcitabine
gmci <- grepl('gemcitibine', colnames(fcgeneexpression))

#rows 365:420(gemcitibine), make dataframe with gmci and biomarkers
