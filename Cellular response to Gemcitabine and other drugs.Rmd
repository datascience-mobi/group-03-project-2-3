...
8. [Drug sensitivity and cell line]  
...
## PCA
PCA is a good method to reduce dimensionality of data. Here we carry out a PCA of the foldchange so the difference after in geneexpression after drug treatment. The fold change is calculated by substracting the untreated from the treated data. We also want to see whether it is maybe tissue assoicated as well.

```{r}
#At first of all we calculate the change of gene expression after treatment
...
After the PCA of the fold change, you can see some clusters. So the fold change is drug associated, which shows that the drug treatment influences the geneexpression. On the other hand the geneexpression of the untreated cells is tissue associated, which also makes sense because same tissue means also a similar geneexpression due to same house-keepinge genes for example. What we cannot see is that the FC is associated to the tissue, which also makes sense because the PCA is carried out for all drugs. If we would have more samples treated with the same drug, maybe we then could see a relation to the tissue.
...
to to one piece of untreated data, a paired T-Test is carried out using the following Hypothesis:
...
However, normalised data should be used for the T-Test. The data is normalised and illustrated in QQ plots as shown below.
...
```{r}
treated_gemcitabine <- dplyr::select(as_tibble(treated_normalized), contains("gemcitibine"))
...
untreated_gemcitabine <- 
  dplyr::select(as_tibble(untreated_normalized), contains("gemcitibine"))
...
absvalfcge_gemcitabine <- 
  dplyr::select(as_tibble(absvalfcgeneexpression), contains("gemcitibine"))
...
# Microsatellite instability - Does MSI status has an effect on the geneexpression after gemcitabine treatment
MSI is defined as a mistake in MMR (Mismatch Repair system). The defect is caused through a downregulation in the gene MLH1 and MSH6 for example. It leads to more mistakes while transcribing the DNA, so more mutations will occur. If we plot the number of mutations against the celllines, MSI-H celllines are supposed to have more mutations, to check this, we do the following plot
...
basic=ggplot(everything, aes(Cell.Line.name, Gemcitabine.drug.sensitivity, colour = factor(Microsatellite), shape = factor(Microsatellite) )) +
   geom_point()
basic + ggtitle("Drug Sensitivity of Gemcitabine treated cell lines according to High Microsatellite Instability")+ xlab("Cell Line")+ylab("Drug Sensitivity")+theme(axis.text.x = element_text(angle = 90, size = 7))
```
...
To answer the question wheter MSI status affects the fold change after gemcitibine treatment, we have to investigate the fold change of our 15 biomarkers for gemcitibine in the different MSI-High and Low celllines. But at first of all, we want to know, whether we can see the effect on the geneexpression of the repair system, so we pick out 2 of the important genes for MSI Instability (MLH1 and MSH6). They belong to the repair system. In MSI-H cancers they should be downregulated, so that the repair system does not work anymore.
...
Here you can see, that there is a difference in the geneexpression in these two genes in MSI-H and MSI-L cancer cells. But does the MSI status even effect the expression of our biomarkers after gemcitibine treatment? 
...
No obvious difference between geneexpression in MSI-H and MSI-L biomarkers, positive prognosis of MSI tumors is probably caused by the recognition of the immune system.
...
#EC50 mean value is 7,003322 for gemcitabine
...
#filter out celllines with 10 highest and 10 lowest EC50 values
sort(NegLogGI50gemcitabine2, decreasing = FALSE)

#cell lines with highest EC50 values are SNB-19, OVCAR-8 ,CAKI-1,NCI-H23 ,HCT-116,
...
Create a matrix for celllines with high EC50 values 
```{r}
...
dimnames(HighEC50) = list( 
    ...
    mutationtype)    
```
Create a matrix for celllines with low EC50 values 
```{r}
...
390.3,2229.0), main= "Celllines with high EC50 fold change values")
...
198.0,564.1,6716.0), main= "Celllines with low EC50 fold change values")

```


EC50 is the concentration for 50% of maximal inhibition of cell proliferation.
Higher values indicate higher sensitivity.The most prominent difference in mutation pattern between the celllines 
