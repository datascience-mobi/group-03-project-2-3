...
```
As can be seen from the above diagram, each batch corresponds to a particular drug treatment. It can also be noted that the mean 
of the boxplots are not levelled. This indicates the lack of normalisation. Thus, the data is normalised and a boxploti is generated again.
...
A T-Test is a statistical hypothesis test which determines if there is a significant difference between the means of two groups. Here our 2 groups are the set of treated and untreated data. Since each piece of treated data corresponds to to one piece of untreated data, a paired T-Test is carried out using the following Hypothesis:
