...
## T-test
A T-Test is statistical hypothesis test used to determine if there is a significant difference between the means of two groups.
In our case the two groups are treated and untreated data. One data in treated data corresponds to one in the untreated data. Thus, a paired T-test is carried out.
...
However, normalised data is necessary for a T-test.

```{r echo=TRUE, results = 'hide', warning=FALSE, message = F}
library("car")
```
```{r}
Gemcitabine_Column_treated= grep("gemcitibine",colnames(treated))
Treated_Cells_Gemcitabine= treated[,Gemcitabine_Column_treated]
Gemcitabine_Column_untreated=grep("gemcitibine",colnames(untreated))
Untreated_Cells_Gemcitabine=untreated[,Gemcitabine_Column_untreated]
```
