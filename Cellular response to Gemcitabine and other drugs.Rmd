...
## PCA
PCA is a good method to reduce dimensionality of data. Here we carry out a PCA of the foldchange so the difference after in geneexpression after drug treatment. We also want to see whether it is maybe tissue assoicated as well.
...
plot(pca_FC, type = "l", main = "PCA of the fold change")
```
A scree plot shows, how much variance a Principal Component explains. As you can see, PC 3 and 4 still explain 10 to 15 percent, we also plot them.
...
ylab = "PC2")
legend("right", inset=c(-0.5,0),levels(tissue_as_factor), pch=20,
...
pheatmap(cbind(geneexpression_MSI_related_untreated_MSIH, geneexpression_MSI_related_untreated_MSIL), cluster_rows = FALSE, cluster_cols = FALSE)
```
